...
if(this->Type != 1 )
throw"Invalid data type: Both values must be numbers to perform multiplication.";
obj.Number = (int)this->Number%(int)a;
obj.Type = 1;
}catch(const char *a){
    cout<<a;
}
...
throw"Invalid data type: Both values must be numbers to perform multiplication.";
obj.Number = (int)this->Number%(int)a.Number;
obj.Type = this->Type;
}catch(const char *a){
    cout<<a;
}
